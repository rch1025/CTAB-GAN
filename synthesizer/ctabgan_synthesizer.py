...
def random_choice_prob_index_sampling(probs,col_idx):
    ...
    for i in col_idx:
        ...
        pp = probs[i] + 1e-6 
        pp = pp / sum(pp)
        # pmf에서 하나 샘플링 (sampled based on given probability mass distribution of categories within the given one-hot-encoding)
...
class Condvec(object):
    ...
    """
    생성자에 들어가는 cond vector를 샘플링하는 클래스

    Variables:
    1) model -> 해당 원핫인코딩 표현에서 강조 표시된 범주의 인덱스를 포함하는 목록 (list containing an index of highlighted categories in their corresponding one-hot-encoded represenations)
    2) interval -> 각각의 원핫인코딩 시작 위치 및 크기를 보유하는 배열 (an array holding the respective one-hot-encoding starting positions and sizes     )
    3) n_col -> 오리지날 데이터의 전체 변수의 개수(total no. of one-hot-encoding representations)
    4) n_opt -> 모든 원핫인코딩 표현에서 고유한 범주의 총 개수 (total no. of distinct categories across all one-hot-encoding representations)
    5) p_log_sampling -> 각각의 one-hot-encoding 표현 내 범주의 확률 대량 분포 로그를 포함하는 목록 (list containing log of probability mass distribution of categories within their respective one-hot-encoding representations)
    6) p_sampling -> 로그 pmf가 아닌 오리지널 pmf (list containing probability mass distribution of categories within their respective one-hot-encoding representations)

    Methods:
    1) __init__() -> takes transformed input data with respective column information to compute class variables
    2) sample_train() -> 모델을 학습할 때, cond vec을 샘플링하는 함수 (used to sample the conditional vector during training of the model)
    3) sample() -> 교육이 끝난 후 데이터를 생성하기 위한 조건부 벡터를 샘플링하는 데 사용됨 (used to sample the conditional vector for generating data after training is finished)
...
class Sampler(object):
    ...
    """
    이 클래스는 조건부 벡터에 따라 변환된 실제 데이터를 샘플링하는 데 사용됨
    ...
    2) sample() -> 샘플링할 행 수, 선택한 열 및 열 내의 범주를 입력으로 받아 그에 따라 실제 레코드를 샘플링 (takes as input the number of rows to be sampled (n), chosen column (col) and category within the column (opt) to sample real records accordingly)
    """
...
def get_st_ed(target_col_index,output_info):
    ...
    """
    분류자가 사용할 변환된 데이터에 따라 대상 열의 시작 및 끝 위치를 얻는 데 사용됩니다. (Used to obtain the start and ending positions of the target column as per the transformed data to be used by the classifier )
...
def weights_init(model):
    ...
    """
    convolution과 batch norm layer 가중치 초기화
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, type={}):
        ...
        for i in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                real = data_sampler.sample(self.batch_size, col[perm], opt[perm]) # 조건에 맞는 실제 데이터 샘플링
                ...
                loss_g = -(torch.log(y_fake + 1e-4).mean()) + cross_entropy
                ...
                if problem_type:
                    ...
                    """classfier의 결과로 생성자를 학습시키기 위해 새롭게 데이터를 생성"""
